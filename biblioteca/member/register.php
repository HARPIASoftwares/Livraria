	...
	<body>
		<form class="cd-form" method="POST" action="#">
			<legend>Insira suas informações</legend>
				...
				<div class="icon">
					<input class="m-user" type="text" name="m_user" id="m_user" placeholder="Usuário" required />
				</div>
				...
				<div class="icon">
					<input class="m-name" type="text" name="m_name" placeholder="Nome Completo" required />
				</div>
				...
				<div class="icon">
					<input class="m-balance" type="number" name="m_balance" id="m_balance" placeholder="Valor Inicial" required />
				</div>
				
				<br />
				<input type="submit" name="m_register" value="Cadastrar" />
		</form>
	</body>
		...
		if(isset($_POST['m_register']))
		{
			...
			else
			{
				...
				if(mysqli_num_rows($query->get_result()) != 0)
					echo error_with_field("Uma conta já está registrada com esse nome!", "m_user");
				else
				{
					...
					$query->execute();
					if(mysqli_num_rows($query->get_result()) != 0)
						echo error_with_field("Uma conta já está registrada com esse e-mail!", "m_email");
					else
					{
