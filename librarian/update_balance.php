	...
	<body>
		<form class="cd-form" method="POST" action="#">
			<legend>Digite as informações necessárias</legend>
				...
				<div class="icon">
					<input class="m-user" type='text' name='m_user' id="m_user" placeholder="Nome de usuário" required />
				</div>
				
				<div class="icon">
					<input class="m-balance" type="number" name="m_balance" placeholder="Quantidade de dinheiro para adicionar" required />
				</div>
				
				<input type="submit" name="m_add" value="Adicionar Valor" />
		</form>
	</body>
	...
	<?php
		if(isset($_POST['m_add']))
		{
			...
			if(mysqli_num_rows($query->get_result()) != 1)
				echo error_with_field("Usuário inválido", "m_user");
			else
			{
