	...
	<body>
		...
		<?php
			...
			else
			{
				...
				echo "<legend>Meus Livros</legend>";
			...
			if(isset($_POST['b_return']))
			{
				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						if($days > 0)
						{
							...
							$query->execute();
							echo '<script>
									document.getElementById("error").innerHTML += "Uma penalidade $ '.$penalty.' foi imposta para reter o seu livro '.$_POST['cb_book'.$i].' por '.$days.' dias após o tempo de entrega acordado.<br />";
				...
				{
					echo '<script>
							document.getElementById("success").innerHTML = "Processo de retorno bem-sucedido! '.$books.' livro";
